...
function extractSectionLines(text: string, sectionNames: string[]) {
  ...
  const normalizedTargets = sectionNames.map((s) => s.toLowerCase());
  ...
  for (const rawLine of lines) {
    ...
    const isHeader = /^[a-zà-ÿ][a-zà-ÿ\s\/-]{2,}:\s*$/i.test(line);
    const matchedTarget = normalizedTargets.some((target) => lower === `${target}:`);
...
function extractAnswerBody(text: string) {
  const lines = text.split(/\r?\n/);
  ...
  for (const rawLine of lines) {
    ...
    if (/^(caveats|caveat|limitations|warnings|missing facts|follow-ups|followups|assumptions|ressalvas|advertencias|fatos faltantes|hechos faltantes):\s*$/i.test(line)) {
      break;
    }

    if (skippingHeaderLine || cleaned.length > 0) {
      cleaned.push(rawLine);
      skippingHeaderLine = false;
    }
  }

  const answer = cleaned.join("\n").trim();
...
function buildConsensus(
  ...
  const caveats = extractSectionLines(chosenText, [
    "Caveats",
    "Caveat",
    "Limitations",
    "Warnings",
  ...
  const followups = extractSectionLines(chosenText, [
    "Missing facts",
    "Follow-ups",
    "Followups",
    "Fatos faltantes",
  ...
  const answer = extractAnswerBody(chosenText);
